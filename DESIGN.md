...
### SignRow
`SignRow.vue` is the atom of the world: a full-width directional sign with a hall-colour tab, a cut label, optional meta/note and a terminal arrow. Props are `href`, `label`, `meta`, `note`, `hall`, `external`, and `size: 'default' | 'door'`. Internal links use `→`; external links use `↗`. Rows are never shorter than `--wf-tap` and the door variant raises the label to `--wf-step-2`.
...
### EntranceDirectory
`EntranceDirectory.vue` is the home entrance and first viewport. Its `variant` prop is `banner | board | directory | record`, defaulting to `banner`, which is what ships in `src/index.md`. `banner` lets the hanging `BannerImage` lead; `board` lets the next event lead as a departure board; `directory` removes display lettering so the halls lead; `record` makes the event count the headline. All variants keep signage rows, hall colour and the illuminated NEXT strip.
